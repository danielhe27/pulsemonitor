// client/src/services/ServicesDashboard.jsx
...
export default function ServicesDashboard({ onSummaryChange }) {
  ...
  useEffect(() => {
    if (onSummaryChange) onSummaryChange(systemSummary);
  }, [systemSummary, onSummaryChange]);
  ...
  const drawerStats = useMemo(() => {
    ...
    if (!rows || rows.length === 0) return { p95: null, min: null, max: null, downPct: null };
  ...
  const timeline = useMemo(() => {
    ...
    return rows.map((r) => ({ ok: r.ok === 1 || r.ok === true, ms: Number(r.ms) || 0 }));
  }, [drawerRows]);

  const insight = useMemo(() => {
    const rows = drawerRows;
    if (!rows || rows.length < 6) {
      return { volatility: "Low", anomaly: "No anomaly detected", recommendation: "Collect more samples to build a baseline." };
    }
  ...
  return (
    <>
        ...
        <div className={styles.panel}>
          ...
          <form onSubmit={addEndpoint} className={styles.form}>
            <div className={styles.field}>
              ...
              <input className={styles.input} value={name} onChange={(e) => setName(e.target.value)} placeholder="Auth API" />
            </div>
            ...
            <div className={styles.field}>
              ...
              <div className={styles.protocolRow}>
                <select className={styles.select} value={defaultProtocol} onChange={(e) => setDefaultProtocol(e.target.value)}>
                  <option value="https://">https://</option>
                  <option value="http://">http://</option>
                </select>
                <input className={styles.input} value={url} onChange={(e) => setUrl(e.target.value)} placeholder="example.com/health" />
              </div>
          ...
          <div className={styles.aiBox}>
            ...
            <div className={styles.aiText}>Calm monitoring. Status first, details on demand. Interface stays quiet until something changes.</div>
          </div>
        </div>
        ...
        <div className={styles.panel}>
          <div className={styles.panelHeader}>
            <div>
              ...
              <div className={styles.panelSub}>Live cards. Trend opens deep telemetry. Auto-generate: {AUTO_GENERATE ? "ON" : "OFF"}</div>
            </div>
          </div>
                  ...
                  <div key={e.id} className={`${styles.card} ${up === null ? "" : up ? styles.cardUp : styles.cardDown}`}>
                    <div className={styles.cardTop}>
                      ...
                      <div className={styles.cardRight}>
                        <button className={styles.smallBtn} onClick={() => runCheck(e.id)} disabled={!!checking[e.id]} type="button">
                          {checking[e.id] ? "Checking..." : "Check"}
                        </button>

                        <button className={styles.smallGhostBtn} onClick={() => openTelemetry(e.id)} type="button">
                          Trend
                        </button>

                        <button className={styles.smallDangerBtn} onClick={() => removeEndpoint(e.id)} type="button">
                          Delete
                        </button>
                      </div>
                    </div>
      ...
      {/* Drawer (still part of Services) */}
      <div className={`${styles.drawerOverlay} ${drawerOpen ? styles.drawerOverlayOpen : ""}`} onClick={() => { if (!drawerPinned) setDrawerOpen(false); }}>
        <div className={`${styles.drawer} ${drawerOpen ? styles.drawerOpen : ""}`} onClick={(e) => e.stopPropagation()}>
          <div className={styles.drawerTop}>
            <div className={styles.drawerTitleRow}>
              ...
              <button className={styles.drawerClose} onClick={() => { if (!drawerPinned) setDrawerOpen(false); }} type="button">
                Close
              </button>
            </div>
            ...
            <div className={styles.drawerControls}>
              ...
              <button className={`${styles.drawerPinBtn} ${drawerPinned ? styles.drawerPinActive : ""}`} onClick={() => setDrawerPinned((v) => !v)} type="button">
              ...
              <div className={styles.drawerHeroLeft}>
                <div className={styles.drawerStatusRow}>
                  ...
                  <div className={styles.drawerStatusMeta}>
                    status {drawerLatest?.status ?? "N/A"} , {safeNum(drawerLatest?.ms)} ms , {drawerLatest?.created_at ? timeAgo(drawerLatest.created_at) : "—"}
                  </div>
                </div>
                ...
                <div className={styles.timelineCard}>
                  ...
                  <div className={styles.timelineRow}>
                    {timeline.map((t, idx) => {
                      ...
                      return (
                        <div key={idx} className={`${styles.timelineDot} ${cls}`} style={{ opacity: alpha }} title={`${t.ok ? "UP" : "DOWN"} • ${t.ms}ms`} />
                      );
                    })}
                  </div>
                </div>
